...
public class Environment {
	 ...
	 * List of litterBins.
	 */
	ArrayList<LitterBin> litterBins;

	/**
	 * Initialises the LitterAgent environment.
	 * 
	 * @param size
	...
	public Environment(int size, CellFactory factory) {
		...
		this.timestep = 0l;
		this.litterBins = new ArrayList<LitterBin>();
	...
	/**
	 * Return the cells a LitterAgent can see.
	 * 
	 * @param pos
	 *            position of the LitterAgent
	 * @param size
	 *            distance the LitterAgent can see
	...
	private Cell getViewerSafeCell(Point pos) {
		Cell c = getCell(pos);
		if (c instanceof WasteBin) {
			c = ((WasteBin) c).clone();
		} else if (c instanceof RecyclingBin) {
			c = ((RecyclingBin) c).clone();
		}
		return c;
	}
